...
//
// ResultsApp API-sindən bütün master data-nı çəkib SQLite-ə yazır.
...
//
// Endpoint-lər:
//   POST /resultsapp-import/preview   → çəkilən sayları qaytarır, yazmır
//   POST /resultsapp-import/run       → bütün data-nı SQLite-ə yazır
//
// Body (hər ikisi üçün, hamısı opsiyonal):
//   {
  ...
  try {
    ...
    res.json({
      ...
      counts: {
        ...
        commission_exercises: snapshot.commission_exercises.length,
        exams:                snapshot.exams.length,
        exam_commissions:     snapshot.exam_commissions.length,
...
// Bütün data-nı çək və SQLite-ə yaz. Topological sırada:
...
router.post("/run", express.json(), async (req, res) => {
  try {
    ...
    // ─── Toplu cavab ─────────────────────────────────────────────────
    const totalInserted = Object.values(reports).reduce((sum, r) => sum + r.inserted, 0);
